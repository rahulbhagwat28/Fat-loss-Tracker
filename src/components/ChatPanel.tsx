...
export default function ChatPanel({ initialWithUserId, className = "" }: ChatPanelProps) {
  const { user } = useAuth();
    ...
    <div className="flex-1 flex flex-col min-w-0 min-h-0">
      <div className="p-3 border-b border-surface-border flex items-center gap-2 flex-shrink-0">
        <button
          ...
          className="p-1.5 -ml-1 rounded-lg text-slate-400 hover:text-white hover:bg-surface-dark"
          ...
          messages.map((m) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isNew && (
                    <span className="block text-[10px] font-bold text-brand-400 tracking-wide uppercase mb-1">New</span>
                  )}
  ...
  return (
    <div className={`flex flex-1 min-h-0 overflow-hidden ${className}`}>
      {showListOnMobile ? (
